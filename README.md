...
Used for ZK-SNARK setup, proof generation, and verification. SnarkJS (by iden3) provides the tools to compile the Circom circuit, generate trusted setup parameters (Groth16 in this case), create proofs, and export the verifier smart contract.
  
- [keccak256-circom](https://github.com/vocdoni/keccak256-circom)
A Circom implementation of the Keccak256 hash function (from circomlib). This was utilized in our circuit to hash the lock transaction data within the proof.
...
- `lockproof.circom` - (ZK circuit for proving the lock event)
- `generate_input.py` – (Python script to format inputs for the circuit and proof generation)
- Solidity contracts – (`Token.sol`, `Lock.sol`, `Mint.sol`)

Approximately 80% of the code was AI-generated, with about 20% hand-written modifications and fixes. As a result, the code may contain bugs or inefficiencies that have not been caught. This is an experimental prototype, and we do not recommend using it in production or mainnet environments. There could be undiscovered vulnerabilities or edge-case issues.

   > ⚠️ Disclaimer: Use this code for educational and testing purposes only. If you plan to build on this demo, thoroughly review and audit the code, and test in isolated environments or testnets before any real-world deployment.
...
1. **ERC-20 Token and Lock Contract on Chain A**: A standard ERC-20 token contract (`Token.sol`) deployed on Chain A, and a `Lock.sol` contract that holds (locks) the tokens. The Lock contract is used to initiate the bridge by locking tokens and recording the details (much like a deposit vault on the source chain).
2. **Circom ZK Circuit for Proof of Lock**:  A zero-knowledge circuit (`lockproof.circom`) built with Circom, which takes the lock event data from Chain A (such as the amount, token contract, and user address, or a transaction hash) and produces a zk-SNARK proof. The circuit uses a Keccak256 hash check internally to confirm that the inputs match the actual lock event data. This proof essentially claims "I know of a valid lock of X tokens by Y" without revealing any unnecessary information beyond the claim itself.
3. **Verifier Contract (and Mint Contract) on Chain B**: A Verifier smart contract (generated by SnarkJS as `Verifier.sol`) is deployed on Chain B. It contains the verifying key corresponding to the Circom circuit and exposes a function to verify proofs on-chain.
...
- No Full Light Client or Block Header Verification: This project does not implement a full on-chain light client or verify entire block headers of Chain A on Chain B. It only proves knowledge of a specific lock transaction.
- Minting Logic Not Automated: The current Mint.sol contract on Chain B is included as a placeholder and does not automatically mint tokens upon proof verification.
- Trusted Setup and Hardcoded Keys: The demo uses Groth16, which requires a trusted setup.

---

## 🚀 Future Improvements and Use Cases
This demo is a starting point. There are several directions in which this prototype could be expanded or applied in the context of blockchain interoperability and zero-knowledge proofs:
- **Full Trustless Bridge Implementation**: To move toward a production zkBridge, one would implement full block header verification. This means using recursive SNARKs or other techniques to prove an entire chain’s state (or a block’s validity) from Chain A to Chain B. Projects like Succinct Labs' work and Berkeley's zkBridge research are exploring such solutions where an updater contract on Chain B maintains verified block headers of Chain A via proofs. Integrating those ideas would make the bridge truly trustless end-to-end, at the cost of much more complex circuits and proofs.
- **Automated Lock-Mint Flow**: Integrate the Mint.sol logic so that as soon as a proof is verified on Chain B, the contract mints the appropriate amount of a wrapped token to the user. This would complete the bridge flow without manual intervention. Additionally, logic to burn or unlock tokens when bridging back from Chain B to Chain A could be added, making the bridge bidirectional.
- **Support for NFTs and Other Assets**: The same concept can be applied to non-fungible tokens (ERC-721/ERC-1155) or other data. For example, a user could lock an NFT on Chain A and mint an equivalent NFT on Chain B after a zk proof. This requires a circuit that can handle NFT identifiers and perhaps metadata proofs. The demo's approach can serve as a template for bridging any asset or even arbitrary messages across chains using proofs.
Enhanced User Experience & Relayers: In a real deployment, off-chain services (or relayers) would likely handle listening for lock events on Chain A, generating proofs, and submitting them to Chain B, so that the user experience is seamless. Future work could involve writing a relayer script or service that automates steps 2–4 of the flow. This could also cover scenarios like batching multiple locks into one proof (with Merkle trees of transactions) to improve efficiency.
